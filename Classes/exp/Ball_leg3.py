    ...
    def bounce(self, mobius_stair):
        ...
        for i, tri in enumerate(self.triangles):
            ...
            if prev_quat is not None:
                ...
                if dot < 0.0:
                    # negate q so it sits on the same hemisphere as prev_quat
                    q = om.MQuaternion(-q.x, -q.y, -q.z, -q.w)

            # If this frame is the start of a new cycle, do a slight blend with prev_quat
            # to remove any small seam noise:
            if prev_quat is not None and (i % len(normals)) == 0:
                # Blend components (lerp then normalize) for a subtle smoothing
            ...
            # Convert quaternion to stable euler degrees for keying
            euler_angles = self.quaternion_to_euler_degrees(q)

            # Final unwrap against prev_angles to avoid any small axis discontinuities
