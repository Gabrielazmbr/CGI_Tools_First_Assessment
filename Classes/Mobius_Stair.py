...
class Mobius_stair:
    ...
    def make_strip(self):
        ...
        l_scale_z = 0
        step = 0.8
        mid_stairs = len(top_face_groups) / 2 - 1
        q_stairs = mid_stairs / 2
        for i, face_range in enumerate(top_face_groups):
            ...
            if i < mid_stairs:
                ...
                if i <= q_stairs + mid_stairs:
                    l_scale_z = round(l_scale_z + step, 2)
                elif i > q_stairs + mid_stairs:
                    l_scale_z = round(l_scale_z - step, 2)
                    if i == len(top_face_groups) - 1:
                        break
        ...
        cmds.setAttr(f"{self.stairs}.translate", 0, 0, 0)
        cmds.select(self.stairs)
